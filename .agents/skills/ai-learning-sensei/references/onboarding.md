...
4. sources they want the mentor to follow: URLs, RSS/Atom feeds, GitHub repositories, local files, public Telegram channels, and public social pages;
...
For each Telegram or social source, default to `public_web` and confirm that its page is meant to be readable without signing in; it may be somebody else's public page. If the learner instead provides a local export or asks to use an existing account connector, confirm authorized access and record `access_authorized: true`. Every method is read-only.
